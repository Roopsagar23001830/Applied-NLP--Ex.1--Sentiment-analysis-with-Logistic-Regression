...
def gradientDescent(x, y, theta, alpha, num_iters):
    ...
    for i in range(num_iters):
        ...
        theta = theta - (alpha/m) * np.dot(x.T, (h - y))      # gradient descent update
...
def extract_features(tweet, freqs, process_tweet=process_tweet):
    ...
    ### START CODE HERE ###
    for word in word_l:
        x[1] += freqs.get((word, 1), 0)  # increment positive count
        x[2] += freqs.get((word, 0), 0)  # increment negative count
    ### END CODE HERE ###
...
def predict_tweet(tweet, freqs, theta):
    ...
    '''
    ### START CODE HERE ###
    # extract the features of the tweet
    x = extract_features(tweet, freqs)
    
    # compute the sigmoid of the dot product between x and theta
...
def test_logistic_regression(test_x, test_y, freqs, theta, predict_tweet=predict_tweet):
    ...
    # convert y_hat and test_y to 1D arrays
